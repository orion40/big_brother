...
localization() {
    ...
    // /map provide the map
    // /amcl_servel provide stats to know where we are

    sub_scan = n.subscribe("scan", 1, &localization::scanCallback, this);
    sub_robot_moving = n.subscribe("robot_moving", 1, &localization::robot_movingCallback, this);

    pub_localization_marker = n.advertise<visualization_msgs::Marker>("localization", 1); // Preparing a topic to publish our results. This will be used by the visualization tool rviz
    pub_localization = n.advertise<geometry_msgs::Point>("goal_to_reach", 1);     // Preparing a topic to publish the goal to reach.
...
void detect_moving_persons() {
    ...
    for (int loop_leg1=0; loop_leg1<nb_moving_legs_detected; loop_leg1++){ //loop over all the legs
        for (int loop_leg2=loop_leg1+1; loop_leg2<nb_moving_legs_detected; loop_leg2++){ //loop over all the legs
            ...
            //...
            goal_to_reach = p;
            pub_localization.publish(goal_to_reach);
