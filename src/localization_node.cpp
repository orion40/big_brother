...
localization() {
    ...
    sub_goal_to_reach = n.subscribe("goal_to_reach", 1, &localization::goal_to_reachCallback, this);

    // communication with rotation_action
    pub_rotation_to_do = n.advertise<std_msgs::Float32>("rotation_to_do", 0);
    sub_rotation_done = n.subscribe("rotation_done", 1, &localization::rotation_doneCallback, this);
    cond_rotation = false;

    // communication with translation_action
    pub_translation_to_do = n.advertise<std_msgs::Float32>("translation_to_do", 0);
    sub_translation_done = n.subscribe("translation_done", 1, &localization::translation_doneCallback, this);
...
void update() {
    ...
    if ( ( new_goal_to_reach ) && ( !cond_translation ) && ( !cond_rotation ) ) {
        ...
        ROS_INFO("(localization_node) /goal_to_reach received: (%f, %f)", goal_to_reach.x, goal_to_reach.y);
        ...
        if ( translation_to_do ) {
            ...
            ROS_INFO("(localization_node) /rotation_to_do: %f", rotation_to_do*180/M_PI);
            ...
            ROS_INFO("(localization_node) /goal_reached (%f, %f)", msg_goal_reached.x, msg_goal_reached.y);
    ...
    if ( new_rotation_done ) {
        ROS_INFO("(localization_node) /rotation_done : %f", rotation_done*180/M_PI);
        cond_rotation = false;
        new_rotation_done = false;

        //the rotation_to_do is done so we perform the translation_to_do
        ROS_INFO("(localization_node) /translation_to_do: %f", translation_to_do);
    ...
    if ( new_translation_done ) {
        ROS_INFO("(localization_node) /translation_done : %f\n", translation_done);
        cond_translation = false;
        new_translation_done = false;
        ...
        ROS_INFO(" ");
        ROS_INFO("(localization_node) waiting for a /goal_to_reach");
    }
...
int main(int argc, char **argv){
    ...
    ROS_INFO("(localization_node) TODO");
